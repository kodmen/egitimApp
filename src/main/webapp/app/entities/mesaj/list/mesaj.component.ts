...
export class MesajComponent implements OnInit {
  ...
  trackId(_index: number, item: IMesaj): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IMesaj[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/mesaj'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
